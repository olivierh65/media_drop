...
(function ($, Drupal, drupalSettings, once) {
  ...
  Drupal.behaviors.mediaDropUpload = {
    attach: function (context, settings) {
      ...
      let userName =
        config.user_name || localStorage.getItem("media_drop_user_name") || "";
    ...
    initDropzone: function ($container, config) {
      ...
      const dropzone = new Dropzone("#media-dropzone", {
        ...
        dictFallbackMessage: Drupal.t(
          "Your browser does not support drag'n'drop"
        ),
        ...
        // Custom accept function to check for duplicates before upload
        // This runs BEFORE the file is uploaded
        // and can prevent the upload by calling done(errorMessage)
            ...
            .done((response) => {
              if (response.exists) {
                ...
                self.setFileStatus(
                  file,
                  "error",
                  response.message || Drupal.t("This file already exists"),
                  true,
                  dropzone
                );
                // Pass error to done() to prevent Dropzone from uploading
                done(""); // Mark as duplicate, but don't show default error
              } else {
                done(); // ✅ upload autorisé
              }
            })
            .fail(() => {
              // Fallback sécurisé : laisser passer
              done();
            });
        },
        ...
        sending: function (file, xhr, formData) {
          ...
          // Check if file is still being checked for duplicates
          if (file.isChecking) {
            console.log("⏳ Still checking for duplicates:", file.name);
            return false;
          }

          // Check if file was marked as duplicate - don't send it
          if (file.isDuplicate) {
            console.log(
              "⏭️ Skipping upload for:",
              file.name,
              "- marked as duplicate"
            );
            return false;
          ...
          if (response.results && response.results[0]) {
            ...
            if (result.success) {
              ...
              if (result.is_duplicate) {
                error_msg = Drupal.t(
                  "This file already exists (same name and size)"
                );
                is_duplicate = true;
              }
              ...
              if (typeof Drupal.announce === "function") {
                Drupal.announce(
                  Drupal.t("Error during upload") + ": " + error_msg,
                  "assertive"
                );
              }
            }
        ...
        error: function (file, errorMessage, xhr) {
          ...
          if (xhr && xhr.status) {
            if (xhr.status === 503) {
              displayError = Drupal.t(
                "Server temporarily unavailable (timeout). Please try again later."
              );
          ...
          if (typeof Drupal.announce === "function") {
            Drupal.announce(
              Drupal.t("Error during upload: @error", {
                "@error": displayError,
              }),
              "assertive"
            );
        ...
        previewTemplate:
          ...
          `
              ...
              <a href="javascript:undefined;" class="dz-remove button button--small button--danger" data-dz-remove>` +
          Drupal.t("Remove from list") +
          `</a>
            </div>
          </div>
        `,
      ...
      dropzone.on("addedfile", function (file) {
        ...
        self.setFileStatus(
          file,
          "uploading",
          Drupal.t("Checking for duplicates..."),
          false,
          dropzone
        );
      });
      ...
      dropzone.on("thumbnail", function (file, dataUrl) {
        console.log("🖼️ Thumbnail generated:", file.name);
      });
    ...
    setFileStatus: function (file, status, message, isDuplicate, dropzone) {
      ...
      const forceShowMark = function ($mark) {
        const styleStr =
          "display: block !important; position: absolute !important; top: 2px !important; right: 2px !important; width: 24px !important; height: 24px !important; z-index: 20 !important;";
      ...
      if (status === "success") {
        ...
        $preview
          .find(".dz-status-text")
          .text(message || Drupal.t("File exists"));
        ...
        const $retryBtn = $preview
          .find(".dz-retry-button")
          .removeClass("dz-hidden");
        ...
        success: function (response) {
          if (response.media) {
            ...
            response.media.forEach(function (media) {
              ...
              $item.append(
                $("<div>", { class: "media-name", text: media.name })
              );

              if (media.subfolder) {
                $item.append(
                  $("<div>", { class: "media-folder", text: media.subfolder })
                );
              }
              ...
              if (config.can_delete) {
                const $deleteBtn = $("<button>", {
                  ...
                  if (
                    confirm(
                      Drupal.t("Are you sure you want to delete this media?")
                    )
                  ) {
                    const deleteUrl = config.delete_media_url.replace(
                      "__MEDIA_ID__",
                      media.id
                    );
                    $.ajax({
